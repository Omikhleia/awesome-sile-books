...
local function parseAnnotationSpec (str, field)
   -- <annotationspecs> ::= <annotationspec> [ ";" <annotationspec> ]
   -- <annotationspec> ::= [ <itemcount> [ ":" <part> ] ] "=" <annotations>
   -- <annotations> ::= <annotation> [ "," <annotation> ]
   -- <annotation> ::= ["] (string) ["]
   local result = {}
   for spec in specs(str) do
      local head, ann = spec:match("^(.-)=(.+)$")
      local itemcount, part = head:match("^(%w+):(%w+)$") or head:match("^(%w+)$")
...
local function apply_annotations (authors, specs, allowed_parts)
   ...
   for _, field in ipairs({ "author", "editor", "translator" }) do
      for _, id in ipairs({ "orcid", "viaf", "isni", "wikidata" }) do
         ...
         if consolidated[annfield] then
            local annotations = parseAnnotationSpec(consolidated[annfield], annfield)
            apply_annotations(consolidated[field], annotations, { id:upper() })
            print(id:upper() .. "s for field '" .. field .. "' in entry '" .. label .. "':")
            pl.pretty.dump(annotations)
            pl.pretty.dump(consolidated[field])
         end
      end
   end
